...
const Lyric = require('../../utils/lyric.js')   // 歌词显示插件
...
Page({
  data: {
    ...
    currentDot: 0,
    playMod: SEQUENCE_MODE  // 播放模式：随机播放、顺序播放、循环播放
  },
  ...
  _init: function () {
    ...
    let duration = currentSong && currentSong.duration   // 歌曲的时长，单位：秒
  ...
  _createAudio: function (playUrl) {
    ...
    wx.onBackgroundAudioPlay(() => {
      this.setData({
        playIcon: 'icon-pause',  // 音乐播放时，按钮图标切换成“暂停”
        cdCls: 'play'
      })
    })
    // 监听音乐暂停。
    wx.onBackgroundAudioPause(() => {
      this.setData({
        playIcon: 'icon-play',  // 音乐暂停时，按钮图标切换成“播放”
        cdCls: 'pause'
      })
    })
    // 监听音乐停止。
    ...
    // 监听播放拿取播放进度
    const manage = wx.getBackgroundAudioManager()   // 获取全局唯一的背景音频管理器 
    manage.onTimeUpdate(() => {
      const currentTime = manage.currentTime   // 当前音频的播放位置
      this.setData({
